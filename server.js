...
const server = http.createServer(async (req, res) => {
  ...
  // ─── /jan-debug — פירוק ספירת הלידים לפי מקור ותאריך (אבחון בלבד) ─────────
  if (req.url?.startsWith('/jan-debug')) {
    try {
      const url = new URL(req.url, 'http://localhost');
      const from = url.searchParams.get('from') || (new Date().getFullYear() + '-01-01');
      const to   = url.searchParams.get('to')   || (new Date().getFullYear() + '-01-31');
      const timeMin = new Date(from);
      const timeMax = new Date(to); timeMax.setHours(23, 59, 59);

      const mondayData = await getAllData();
      ...
      const dayKey = (dateStr) => {
        const d = new Date(String(dateStr).replace(' ', 'T'));
        return isNaN(d) ? 'invalid' : d.toISOString().slice(0, 10);
      };

      // אותם ארבעה מקורות כמו ב-/stats
      const fromBoard = (mondayData.leads || []).filter(i => {
        if (i._approxDate) return false;
        const v = gv(i, 'date_mm00ds06');
        return v && inRange(v);
      });
      const fromStepup   = (mondayData.stepup   || []).filter(i => inRange(i.created_at));
      const fromSales    = (mondayData.sales    || []).filter(i => { const v = gv(i, 'date_mm00jx0c'); return v && inRange(v); });
      const fromCoaching = (mondayData.coaching || []).filter(i => inRange(i.created_at));

      // דדופ לפי שם באותו סדר כמו /stats — כדי לראות כמה כל מקור באמת מוסיף
      const seen = new Set();
      const addUnique = (arr) => {
        let added = 0;
        arr.forEach(i => { const n = (i.name || '').trim().toLowerCase(); if (!seen.has(n)) { seen.add(n); added++; } });
        return added;
      };
      const boardCount    = addUnique(fromBoard);
      const stepupCount   = addUnique(fromStepup);
      const salesCount    = addUnique(fromSales);
      const coachingCount = addUnique(fromCoaching);
      const total = boardCount + stepupCount + salesCount + coachingCount;

      // התפלגות לפי יום בבורד הלידים — לזהות ספייק של יום אחד (הגירה)
      const byDay = {};
      fromBoard.forEach(i => { const k = dayKey(gv(i, 'date_mm00ds06')); byDay[k] = (byDay[k] || 0) + 1; });
      ...
      res.end(JSON.stringify({
        ...
        range: { from, to },
        total,
        bySource_afterDedup: { leadsBoard: boardCount, stepup: stepupCount, sales: salesCount, coaching: coachingCount },
        rawCounts_beforeDedup: { leadsBoard: fromBoard.length, stepup: fromStepup.length, sales: fromSales.length, coaching: fromCoaching.length },
        topDatesInLeadsBoard: topDays.map(([date, count]) => ({ date, count })),
      }, null, 2));
    } catch(e) {
      res.writeHead(500, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: false, error: e.message }));
